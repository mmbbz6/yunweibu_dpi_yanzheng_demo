...
def _str2mirsecond_iucs(str):
    #print "_str2mirsecond_iucs:", str
    #print "_str2mirsecond_iucs:", datetime.datetime.strptime(str[:-1], '%Y-%m-%d %H:%M:%S.%f')
...
def _write2file(csfb, iucs, msisdn, st, et, lai, fail):
    with open(args['output'], 'a') as f:
        ...
        print "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
	print iucs[msisdn], iucs[st], iucs[et]
...
def _is_matched(row, msisdn, st, et, lai, fail, filename, rownum, type):
    ...
    if row[msisdn] == '' or row[st] == '' or row[et] == '':
        pass
        #logger.warning('类型%s的文件%s第%s行字段有空：msisdn:%s,starttime:%s,endtime:%s.详细信息是%s' % (type, filename, rownum, row[msisdn], row[st], row[et], str(row)))#cm1
    else:
        if row[msisdn] in CSFB_sampling_dict:
            try:
                d11 = _str2mirsecond(CSFB_sampling_dict[row[msisdn]][0])
		#print "d11:",CSFB_sampling_dict[row[msisdn]][0],d11
                d12 = _str2mirsecond(CSFB_sampling_dict[row[msisdn]][1])
		#print "d12:",CSFB_sampling_dict[row[msisdn]][1],d12
		#print "d21:row[st]",row[st]
		...
		#print "11111pppp"
            ...
            #logger.info('%s not in dict' % row[msisdn])#cm1
            pass

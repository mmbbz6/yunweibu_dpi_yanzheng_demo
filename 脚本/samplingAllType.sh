...
for line in `ls $DIR|awk 'BEGIN{FS="_";OFS="|"} {split($10,a,"-") array[a[1]]++} END{for(i in array) print i,array[i]}'`
do
	...
	if [ $n -lt $FILENUM ];then
		...
		echo "*$t ok"
		for file in `ls $DIR|awk -v t=$t 'BEGIN{FS="_"} {split($10,a,"-"); if (a[1]==t) {print $0}}'|shuf -n$FILENUM`
		do

...
rm -rf sampling.txt
for i in `ls $DIR|awk -v t=$TYPE 'BEGIN{FS="_"} {split($10,a,"-"); if (a[1]==t) {print $0}}'|shuf -n$FILENUM`
do
        echo $i
...
rm -f IuCS.list
ls $DIR|awk -v d=$DIR 'BEGIN{FS="_";OFS="/"} {split($10,a,"-"); if (a[1]=="IuCS") {print d,$0}}' >> IuCS.list
